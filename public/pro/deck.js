/* 練息場 Pro Saleskit（綜合版）— 前台渲染 + 內容覆寫 + 編輯橋接
   1. 先用 data.js 把動態區塊（五感卡／課程牆／年度排程／滿版照片）畫出來
   2. 再向 /api/content 取得後台覆寫（文字 + 圖片），套用上去
   3. 若被 /pro/mng 以 iframe 載入，開啟編輯模式並與父視窗溝通            */
(function () {
  'use strict';
  var D = window.PRO_DATA;
  var senseByKey = {};
  ...
  function loadContent() {
    return fetch('/api/pro/content', { cache: 'no-store' })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (j) { applyOverrides(j); return j; })
      .catch(function () { return null; });
  }

  /* ---------- 3. 編輯模式（由 /pro/mng 以 iframe 載入時啟用）---------- */
  ...
  function renderRoi() {
    var box = document.getElementById('roi');
    if (!box) return;
    var ins = box.querySelectorAll('input[data-roi]');
    var outY = document.getElementById('roiYear');
    var out3 = document.getElementById('roi3y');
    function fmt(n) { return 'NT$ ' + Math.round(n).toLocaleString('en-US'); }
    function calc() {
      var v = {};
      ins.forEach(function (i) { v[i.getAttribute('data-roi')] = parseFloat(i.value) || 0; });
      // 每年可省 = 員工數 × 離職率下降% × 平均年薪(萬) × 離職成本倍數
      var year = v.head * (v.drop / 100) * (v.pay * 10000) * v.mult;
      outY.textContent = fmt(year);
      out3.textContent = '三年累積約 ' + fmt(year * 3);
    }
