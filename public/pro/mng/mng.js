...
(function () {
  ...
  fetch('/api/pro/me').then(function (r) { return r.json(); })
  ...
  $('#btnLogout').addEventListener('click', function () {
    fetch('/api/pro/logout', { method: 'POST' }).then(function () { setLogin(true); });
  });
  ...
  $('#btnReset').addEventListener('click', function () {
    ...
    disarmReset();
    fetch('/api/pro/content', { method: 'DELETE' }).then(function (r) {
  ...
  function handleFile(file) {
    ...
    compress(file, 1800, 0.82).then(function (blob) {
      return fetch('/api/pro/upload', { method: 'POST', headers: { 'Content-Type': 'image/jpeg' }, body: blob });
    }).then(function (r) {
