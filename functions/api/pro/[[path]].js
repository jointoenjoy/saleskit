/* 練息場 Chill Saleskit — 後台 API（Cloudflare Pages Function）
   掛在 /api/pro/* ；共用舊版 saleskit 的 DECK_KV 與 EDIT_KEY，不用另外開 secret。
   KV 鍵一律以 chill: 開頭，和舊版的 deck:latest／deck:prev 不衝突。

     GET    /api/pro/content   讀內容覆寫（公開，前台載入時打）
     PUT    /api/pro/content   存內容覆寫（需登入）
     DELETE /api/pro/content   還原預設（需登入）
     POST   /api/pro/login     以 EDIT_KEY 登入，發簽章 cookie
     POST   /api/pro/logout    清 cookie
     GET    /api/pro/me        查詢登入狀態
     POST   /api/pro/upload    上傳圖片進 KV（需登入）
     GET    /api/pro/img/:id   取出上傳的圖片                            */

const CONTENT_KEY = 'pro:content';
const IMG_PREFIX = 'pro:img:';
const COOKIE = 'pro_admin';
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (route === 'upload' && m === 'POST') {
      ...
      return json({ ok: true, url: `/api/pro/img/${id}` });
    }
