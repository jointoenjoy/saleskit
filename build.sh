...
#   /pro         Pro 綜合版簡報（Chill × Well），/pro/mng 後台
...
import re, sys, os
bad = []
for f in ('public/chill/data.js', 'public/chill/index.html', 'public/pro/data.js', 'public/pro/index.html'):
...
cp -R public/. dist/
# Pages Functions（雲端存檔 API + /api/chill/* + /api/pro/*）一起帶進部署資料夾
